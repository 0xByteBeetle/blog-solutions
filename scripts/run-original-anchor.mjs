...
if(!['borsh-profile','borsh-proposal','zero-copy','zero-copy-original','anchor-accounts'].includes(name))throw new Error('Choose borsh-profile, borsh-proposal, zero-copy, zero-copy-original, or anchor-accounts');
...
const program=name==='anchor-accounts'?'solana_accounts':name==='zero-copy'?'zero_copy_deep_dive':'borsh_deep_dive';
...
try{
 ...
 fs.writeFileSync(`${root}/verification/${name}.json`,JSON.stringify({date:new Date().toISOString(),project:`examples/solana/${name}`,status:result,sourceHashes,command:`node scripts/run-original-anchor.mjs ${name}`,changes:'Only temporary program IDs, provider/ports/wallet, and test launcher changed; original instructions, account layout, and test body preserved. TypeScript tests run transpile-only, not a type-check claim.',anchorVersion:spawnSync('anchor',['--version'],{encoding:'utf8'}).stdout.trim()},null,2)+'\n');
