...
const altReady=process.argv.includes('--alt-ready');
if(altReady&&(!regression||name!=='wallet-token-balances-original'))throw new Error('--alt-ready requires the wallet regression');
...
if(regression&&name==='wallet-token-balances-original'){
 ...
 const original=fs.readFileSync(project+`/scripts/${altReady?'client-alt-ready.ts':'client.ts'}`,'utf8');
 ...
 // Select the explicit corrected variant only with --alt-ready; otherwise use the original.
...
const execution=[];
function run(cmd,args,options={}){const r=spawnSync(cmd,args,{cwd:project,encoding:'utf8',env:{...process.env,TS_NODE_TRANSPILE_ONLY:'true',BLOG_PREPARED_ALT:preparedAlt?'1':'0',BLOG_ALT_READY:altReady?'1':'0'},maxBuffer:30*1024*1024,...options});if(r.stdout)process.stdout.write(r.stdout);if(r.stderr)process.stderr.write(r.stderr);if(cmd==='anchor'||cmd.endsWith('/tsc'))execution.push({command:[cmd,...args].join(' '),exitCode:r.status,stdout:r.stdout,stderr:r.stderr});if(r.status!==0)throw new Error(`${cmd} failed (${r.status})`);return r;}
...
if(altReady){
 const variant='examples/solana/wallet-token-balances-original/scripts/client-alt-ready.ts';
 sourceHashes[variant]=crypto.createHash('sha256').update(fs.readFileSync(root+'/'+variant)).digest('hex');
}
let result='failed';
...
const changes=altReady
 ? 'Authorized client-alt-ready.ts variant: poll confirmed ALT contents until all required addresses exist and response.context.slot exceeds lastExtendedSlot; use confirmed preflight and minContextSlot for the consuming transaction. Original client.ts and Rust program preserved. Test-only creator uses a finalized recent slot; fixtures use disposable local mints/wallets. No pre-population unless --prepared-alt is explicitly selected.'
 : regression
...
try{
 ...
 if(regression&&name==='wallet-token-balances-original'){
  ...
  if(altReady)run(deps+'/.bin/tsc',['--noEmit','--skipLibCheck','--esModuleInterop','--target','es2020','--module','commonjs','scripts/client-alt-ready.ts']);
 ...
 const evidenceName=`${name}${regression?'-regression':''}${preparedAlt?'-prepared-alt':''}${altReady?'-alt-ready':''}`;
